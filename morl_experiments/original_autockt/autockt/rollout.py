...
def rollout(agent, env_name, num_steps, out="assdf", no_render=True):
    ...
    obs_reached = []
    obs_nreached = []
    obs_reached_achieved = []   # actual circuit output when done=True (so output >= target for Yes)
    obs_nreached_achieved = []  # actual circuit output when done=False
    action_array = []
    action_arr_comp = []
    rollout_steps = 0
    reached_spec = 0
    while rollout_steps < args.num_val_specs:
        ...
        while not done and steps < args.traj_len:
            ...
            steps += 1
            state = next_state
        # state[0:4] = cur_spec_norm (achieved), state[4:8] = specs_ideal_norm (target)
        norm_achieved_spec = state[0:spec_num]
        achieved_spec = unlookup(norm_achieved_spec, norm_spec_ref)
        ...
        pickle.dump(obs_reached_achieved, open("opamp_obs_reached_achieved_test","wb"))
        pickle.dump(obs_nreached_achieved, open("opamp_obs_nreached_achieved_test","wb"))
