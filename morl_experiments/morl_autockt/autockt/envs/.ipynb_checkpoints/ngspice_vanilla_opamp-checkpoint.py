...
class TwoStageAmp(gym.Env):
    ...
    def __init__(self, env_config):
        ...
        self.multi_objective = env_config.get("multi_objective", False)  # New MO flag
        ...
        if self.multi_objective:
            self.reward_space = spaces.Box(low=np.array([-np.inf, -np.inf, -np.inf, -np.inf]), 
    ...
    def step(self, action):
        ...
        if self.multi_objective:
            reward = self.multi_objective_reward(self.cur_specs, self.specs_ideal)
        else:
            reward = self.reward(self.cur_specs, self.specs_ideal)
        done = False

        if self.multi_objective:
            if np.all(np.array(reward) >= -0.02):  # All objectives within threshold
                done = True
                print('-'*10)
                print('params = ', self.cur_params_idx)
                print('specs:', self.cur_specs)
                print('ideal specs:', self.specs_ideal)
                print('multi-objective rewards:', reward)
                print('-'*10)
        else:
            if (reward >= 10):
                done = True
    ...
    def multi_objective_reward(self, spec, goal_spec):
        '''
        Multi-objective reward function: returns vector of individual objective rewards
        Each objective: ugbw, gain, phm, ibias_max
        '''
        rel_specs = self.lookup(spec, goal_spec)
        obj_rewards = []
        ...
        for i, rel_spec in enumerate(rel_specs):
            ...
            if rel_spec < 0:
                obj_reward = rel_spec  # Penalty for not meeting spec
            else:
                obj_reward = 10.0 if rel_spec > 0.02 else rel_spec  # Bonus for exceeding spec
                
            obj_rewards.append(obj_reward)
        
        return np.array(obj_rewards, dtype=np.float32)
