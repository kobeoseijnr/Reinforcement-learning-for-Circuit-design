...
def run(args, parser):
    ...
    rollout(agent, args.env, num_steps, args.out, args.no_render, args.num_val_specs, args.traj_len)

def unlookup(norm_spec, goal_spec):
    spec = -1*np.multiply((norm_spec+1), goal_spec)/(norm_spec-1) 
    return spec

def rollout(agent, env_name, num_steps, out="assdf", no_render=True, num_val_specs=50, traj_len=60):
    if hasattr(agent, "local_evaluator"):
        #env = agent.local_evaluator.env
        env_config = {"generalize":True,"num_valid":num_val_specs, "save_specs":False, "run_valid":True}
    ...
    while rollout_steps < num_val_specs:
        ...
        if done == True:
            print(done)  # Print True when spec is reached (as per README format)
            reached_spec += 1
